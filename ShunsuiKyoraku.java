 ...
 * @version 05/12/2017
 */
public class ShunsuiKyoraku extends Player{
    protected final String newName = "ShunsuiKyoraku"; //Overwrite this variable in your player subclass

    /**Do not alter this constructor as nothing has been initialized yet. Please use initialize() instead*/
    public ShunsuiKyoraku() {
        super();
        playerName = newName;
    }
     ...
     * Simple Agent using basic heuristics
     * Basic heuristic to find the neighbor that has the most highest face cards occurences
     * such as jack's, queen's, king's, and ace's
     */
    public Action makeAction() {
        int[] faceCards = new int[graph[currentNode].getNeighborAmount()];
        int face;
        ...
        for (int i = 0; i < faceCards.length; i++) {
            ...
            if (possible.size() != 0) {
                for (int j = 0; j < possible.size(); j++) {
                    if (possible.get(j).getRank() > 10 || possible.get(j).getRank() == 1)
                        face++;
                }
                faceCards[i] = face;
            }
        ...
        int maxFaceCards = 0;
        for (int k = 1; k < faceCards.length; k++)
            if (faceCards[k] > faceCards[maxFaceCards])
                maxFaceCards = k;
        int neighbor = graph[currentNode].getNeighbor(maxFaceCards).getNodeID();
